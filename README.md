...
- **阶段2（YOLOv8n-cls）**：对手腕裁剪区域进行 5 类分类（knife/axe/hammer/stick/none），采用独立 Sigmoid 阈值判定（≥0.75 才判为危险品），避免 argmax 导致空手误报。
...
|------|-------|----------|------|
| YOLOv8n-Pose（阶段1） | 0.921 | — | 人体检测 + 手腕关键点 |
| YOLOv8n-cls（阶段2） | 0.995 | 0.956 | 5 类分类（含 none） |
...
**测试环境**：GPU (NVIDIA)，阶段1 训练集 57,000 张，验证集 5,000 张；阶段2 训练集约 16,000 张，5 个类别。
...
- **来源**：[Roboflow Universe](https://universe.roboflow.com/) 公开武器检测数据集
- **类别**：knife（刀具）、axe（斧头）、hammer（锤子）、stick（棍棒）、none（安全/负样本）
- **规模**：约 16,000+ 张，226×226 像素手部局部裁剪图
- **负样本**：none 类别为人工采集合成（空手、握拳、手机、水杯等）
...
2. **数据集规模**：阶段2数据集仍有扩充空间，特别是安全/负样本类别（none）的多样性不足。
3. **跨场景泛化**：当前测试主要在受控场景下进行，跨场景（不同光照、不同摄像头角度）泛化能力未充分验证。
4. **运动模糊鲁棒性**：运动模糊测试通过率为 80%，仍有提升空间，计划增加 MotionBlur 数据增强比例。
5. **推理速度优化**：尚未进行 TensorRT 导出和多路摄像头并发压测。
6. **类别扩展**：当前仅支持 5 类分类，计划扩展至枪支、管制刀具等更多危险物品类别。
